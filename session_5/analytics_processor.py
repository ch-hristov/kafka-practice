"""
Analytics Processor Consumer with Observability

Processes user activity events and aggregates analytics with logging and metrics.
Shows conversion funnels, revenue tracking, and user behavior insights.
"""
...
STATS_INTERVAL = int(os.getenv("STATS_INTERVAL", "50"))
LOG_LEVEL = os.getenv("LOG_LEVEL", "INFO")

# Setup logging
logger = setup_logging("analytics_processor", level=LOG_LEVEL)
...
def extract_metrics(event):
    """Extract and aggregate metrics from event"""
    ...
    if event_type in funnel_map:
        aggregations["funnel"][funnel_map[event_type]].add(user_id)
    ...
    if event_type == "purchase":
        ...
        if cart_value > 0:
            ...
            increment("revenue_total", value=int(cart_value * 100))
...
def print_statistics():
    """Print aggregated statistics in a clear format"""
    print("\n" + "=" * 80)
